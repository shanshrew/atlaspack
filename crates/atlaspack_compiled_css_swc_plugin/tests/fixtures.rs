...
fn transform_source(src: String, file_name: &str, syntax: Syntax) -> String {
  ...
  let has_jsx_source_pragma = leading
    ...
    || trailing
      ...
      .flat_map(|v| v.iter())
      .any(|c| c.text.contains("@jsxImportSource @compiled/react"));
  swc_noop_plugin::set_jsx_import_source_compiled(has_jsx_source_pragma);
  ...
  swc_noop_plugin::set_jsx_classic_pragma_local(classic_local);
  ...
  let out_program = HANDLER.set(&handler, || swc_noop_plugin::process_transform(program));
