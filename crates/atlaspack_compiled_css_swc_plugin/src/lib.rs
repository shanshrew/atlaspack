...
pub(crate) const CSS_PREFIX: &str = "_";
...
impl AtomicCssCollector {
  ...
  fn create_runtime_var_for_expr(&mut self, expr: &Expr, wrap_iife: bool) -> String {
    ...
    let var_name = format!("--{}", self.hash_class_name(&["ix", &key]));
    self
      .tmp_runtime_entries
...
impl VisitMut for AtomicCssCollector {
  ...
  fn visit_mut_expr(&mut self, n: &mut Expr) {
    // Only intercept styled.* calls/tags; css/cssMap will be handled at JSX and const collection
    match n {
      Expr::Call(call) => {
...
pub fn process_transform(program: Program) -> Program {
  ...
  let mut v = AtomicCssCollector::default();
...
pub fn apply_compiled_atomic(program: &mut Program) {
  let mut v = AtomicCssCollector::default();
  program.visit_mut_with(&mut v);
}
