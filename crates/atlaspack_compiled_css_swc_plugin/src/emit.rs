...
use crate::{AtomicCssCollector, CSS_PREFIX, base_encode36};
...
impl AtomicCssCollector {
  ...
  pub fn hash_class_name(&self, tokens: &[&str]) -> String {
  ...
  pub fn emit_rule_input<'a>(&mut self, input: RuleInput<'a>, out_classes: &mut Vec<String>) {
    ...
    let class_name =
      self.hash_class_name(&[input.suffix, input.prop_kebab, input.value_text.as_ref()]);
    let mut hasher = Xxh3Builder::new().build();
    hasher.write(input.suffix.as_bytes());
    hasher.write(&[0]);
    hasher.write(input.prop_kebab.as_bytes());
    hasher.write(&[0]);
    hasher.write(input.value_text.as_ref().as_bytes());
    hasher.write(&[0]);
    ...
    let mut value_text_cow = input.value_text;
